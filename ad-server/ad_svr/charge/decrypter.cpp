...
extern "C" {
int TestWinningPrice(const char* enctription_str) {
  const char kEncryptionKey[] = {
0x03,0x38,0x27,0x70,0x7D,0xCB,0xBB,0x2F,0x82,0x6D,0x1B,0xA8,0x85,0x31,0x62,0x77,0xDA,0xE8,0x54,0x7F,0x4E,0xF5,0x87,0x29,0x59,0xF0,0x24,0xF4,0x54,0x27,0x34,0x4E
  };
  const char kIntegrityKey[] = {
0x1B,0x73,0x5C,0x68,0xDB,0xA7,0x7E,0xD7,0xD7,0xF4,0xAB,0x96,0xE1,0xF2,0xB5,0x8F,0x93,0xCA,0x69,0xCD,0xE9,0x7E,0xB1,0x86,0xB0,0xEA,0x12,0xE5,0x45,0xEA,0x25,0x1C
  };
  ...
  const string kB64EncodedValue(enctription_str);
  ...
  bool success = DecryptWinningPrice(
      encrypted_value, encryption_key, integrity_key, &value);
  if (success) {
...
int main(int argc, char** argv) {
  int64 price = TestWinningPrice(argv[1]);
  printf("%lld",price);
  char price_str[128];
  snprintf(price_str,128,"%lld",price);
  return price;
}
